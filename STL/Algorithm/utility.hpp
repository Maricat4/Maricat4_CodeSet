...
typedef std::unordered_map<std::string,int> maptype3;
typedef std::unordered_map<int,int> maptype4;
template<>
...
//函数模板全特化
template<>
inline void _showContainerContent<maptype3>(maptype3 a,const char *str){
    std::cout<<str<<":";
    if(a.size() == 0) {
        std::cout<<"无元素"<<std::endl;
        return;
...
//函数模板全特化
template<>
inline void _showContainerContent<maptype4>(maptype4 a,const char *str){
    std::cout<<str<<":";
    if(a.size() == 0) {
        std::cout<<"无元素"<<std::endl;
        return;
